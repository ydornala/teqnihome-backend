...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Teqnihome Backend account.<br>
        ...
        &mdash; Teqnihome Team
      `
      return sendMail({ toEmail: email, subject: 'Teqnihome - Password Reset', content })
    })
...
export const show = ({ params: { token } }, res, next) =>
  PasswordReset.findOne({ token })
    ...
    .then(success(res))
    .then(res.sendFile(__dirname + '/password-resets.html'))
    .catch(next)
